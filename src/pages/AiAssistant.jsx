...
- תשובה לאחר מידע: ממוקדת, ניתנת לסריקה (כותרות/בולטים כשמתאים), בלי "הקדמות".${vehicleContext}`;
      ...
      setMessages(prev => [...prev, {
        role: 'assistant',
        // Display cap raised 2500 → 8000 chars. The old 2500 matched the
        // original 700-token budget; once max_tokens went to 1500 a full
        // Hebrew answer (~5-6k chars) got hard-cut mid-sentence here even
        // though the API returned it whole. 8000 never clips a legitimate
        // 1500-token reply and still guards against a runaway payload.
        content: sanitize(aiText).slice(0, 8000),
        ts: Date.now(),
        vehicleId: selectedVehicleId,
