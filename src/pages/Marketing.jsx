...
export default function Marketing() {
  ...
  return <div className="cm-site" dir="rtl">
    ...
    <main id="cm-main">
      {home && <>
        <section className="cm-hero"><MarketingHeroBackground /><div className="cm-wrap cm-hero-grid"><div className="cm-hero-copy"><h1>תזכורות ותחזוקה{' '}<br /><em>לכל כלי שיש לכם.</em></h1><p className="cm-hero-lead">תזכורות לטסט, לביטוח ולטיפולים.<br />כל המועדים, המסמכים וההוצאות נשמרים לצד כל כלי.</p><div className="cm-actions"><a href="#download" className="cm-button cm-gold">להורדת האפליקציה <ArrowLeft size={19} /></a><a href="#check" className="cm-text-link">קודם בודקים את הרכב <Search size={17} /></a></div></div><div className="cm-hero-visual"><div className="cm-orbit" /><MarketingPhone src="/marketing/dashboard.webp" alt="לוח הבקרה באפליקציה עם נתוני הדגמה" priority /><span className="cm-photo-note">מסך מהאפליקציה במסגרת להמחשה</span></div></div></section>
    ...
    <footer className="cm-footer"><div className="cm-wrap"><Link className="cm-brand" to="/website"><img src={logo} width="35" height="35" alt="" />Car Reminder</Link><nav aria-label="מידע וקשר"><Link to="/Contact">יצירת קשר</Link><Link to="/PrivacyPolicy">פרטיות</Link><Link to="/TermsOfService">תנאי שימוש</Link><Link to="/Auth">כניסה לחשבון</Link></nav><span>כל מה שחשוב לכלי התחבורה שלך.</span></div></footer>
