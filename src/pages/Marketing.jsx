...
const CARD_HEIGHT = 473;
const categoryArt = {
  offroad: { title: 'אופנועים וכלי שטח', text: 'שעות מנוע, טיפולים וכל מה שצריך לפני היציאה לשטח.', image: '/marketing/cards/hero-ktm.webp' },
  trucks: { title: 'משאיות ורכבי עבודה', text: 'תיקי רכב, נהגים ומסמכים במקום אחד.', image: '/marketing/cards/category-full-trailer.webp' },
  'heavy-equipment': { title: 'טרקטורים וכלי צמ״ה', text: 'תיעוד שימוש, תחזוקה ותוקף תסקירים.', image: '/marketing/cards/hero-loader.webp' },
  'classic-cars': { title: 'רכבי אספנות', text: 'שומרים את היסטוריית הטיפולים והשיקום.', image: '/marketing/cards/category-classic-beetle.webp' },
  'inactive-vehicles': { title: 'רכבים לא פעילים', text: 'בודקים את המידע ומרכזים את התיעוד.', image: '/marketing/cards/category-inactive-car.webp' },
};
const categoryCards = [
  ...specialtyPages.map(page => ({ ...categoryArt[page.slug], slug: page.slug, href: '/website/' + page.slug })),
  { slug: 'jet-skis', href: '/website/vessels', title: 'אופנועי ים', text: 'שעות מנוע, טיפולים ומסמכים לקראת היציאה למים.', image: '/marketing/cards/hero-jet-ski.webp' },
  { slug: 'motorboats', href: '/website/vessels', title: 'סירות מנוע', text: 'מרכזים את פרטי הסירה, שעות המנוע ומועדי התחזוקה.', image: '/marketing/cards/category-motorboat.webp' },
  { slug: 'sailing-yachts', href: '/website/vessels', title: 'יאכטות מפרשיות', text: 'מסמכים, מועדי חידוש ותיעוד טיפולים בתיק אחד ליאכטה.', image: '/marketing/cards/category-sailing-yacht.webp' },
];

function CategoryImage({ src }) {
  return <picture>
    <source type="image/avif" srcSet={src.replace(/\.webp$/, '.avif')} />
    <img src={src} alt="" width={CARD_WIDTH} height={CARD_HEIGHT} loading="lazy" decoding="async" />
  </picture>;
}
...
// FAQPage schema only, on /website/business (see the useEffect below).
const businessFaqs = [
  ['איך מצטרפים עם עסק?', 'חשבון עסקי נפתח לאחר הגשת בקשה ואישור. בעמוד העסקים אפשר להכיר את היכולות ולפנות לגבי התאמה לצוות שלכם.'],
  ...
  return <div className="cm-site" dir="rtl">
    ...
    <main id="cm-main" tabIndex={-1}>
      {home && <><MarketingJsonLd data={mobileAppJsonLd()} />
        ...
        <section className="cm-section cm-specialties"><div className="cm-wrap"><span className="cm-kicker">לכל כלי יש דרך עבודה משלו</span><h2>ניהול אופנועים, משאיות וכלי צמ״ה{' '}<br />לא רק רכב פרטי.</h2><p className="cm-specialties-list">רכב פרטי ומסחרי, אופנועים וקטנועים, אנדורו ומוטוקרוס, טרקטורונים ורכבי שטח, משאיות ורכבי עבודה, אוטובוסים ומיניבוסים, טרקטורים וכלי צמ״ה, מחפרים, שופלים, מלגזות, מכבשים ומנופים, רכבי אספנות, נגררים וקראוונים, וכלי שיט: אופנועי ים, סירות מנוע, סירות גומי ומפרשיות.</p><div className="cm-specialties-grid">{categoryCards.map(card => <Link key={card.slug} to={card.href}><div className="cm-category-art"><CategoryImage src={card.image} /></div><h3>{card.title}</h3><p>{card.text}</p><span>לפרטים <ArrowLeft size={17} /></span></Link>)}</div></div></section>
        {business}
        <section className="cm-section"><div className="cm-wrap"><span className="cm-kicker">עוד דרכים להכיר את Car Reminder</span><h2>בדיקת רכב{' '}<br /><em>ותזכורות לטסט וביטוח.</em></h2><div className="cm-guides cm-guides-2">
