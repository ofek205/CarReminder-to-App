...
export default function Marketing() {
  ...
  return <div className="cm-site" dir="rtl">
    ...
    <main id="cm-main">
      {home && <>
        <section className="cm-hero"><MarketingHeroBackground /><div className="cm-wrap cm-hero-grid"><div className="cm-hero-copy"><h1>תזכורות לרכב.{' '}<br /><em>ניהול רכב ומסמכים מסודרים.</em></h1><p className="cm-hero-lead">תזכורות לטסט, לביטוח ולטיפולים.<br />כל המועדים, המסמכים וההוצאות נשמרים לצד הרכב.</p><div className="cm-actions"><a href="#download" className="cm-button cm-gold">להורדת האפליקציה <ArrowLeft size={19} /></a><a href="#check" className="cm-text-link">קודם בודקים את הרכב <Search size={17} /></a></div></div><div className="cm-hero-visual"><div className="cm-orbit" /><MarketingPhone src="/marketing/dashboard.webp" alt="לוח הבקרה באפליקציה עם נתוני הדגמה" priority /><span className="cm-photo-note">מסך מהאפליקציה במסגרת להמחשה</span></div></div></section>
        ...
        <section className="cm-section"><div className="cm-wrap cm-special-grid"><article id="vessels" className="cm-vessel"><Ship size={36} /><span className="cm-kicker">גם על המים</span><h2>ניהול כלי שיט{' '}<br />עם מקום משלו.</h2><p>שמרו מסמכים, מועדים ושעות מנוע תחת כלי השיט המתאים.</p><Link to="/website/vessels" className="cm-text-link">לניהול כלי שיט <ArrowLeft size={18} /></Link></article><article className="cm-intelligence"><Sparkles size={32} /><span className="cm-kicker">פחות עבודה ידנית</span><h2>סריקת מסמכי רכב{' '}<br />ועזרה בשאלות עליו.</h2><p>סריקת מסמכים ועוזר חכם כחלק מניהול הרכב. בודקים את הפרטים שמתקבלים ומאשרים לפני שממשיכים.</p><ol><li>מעלים מסמך</li><li>בודקים את הפרטים שחולצו</li><li>מאשרים ושומרים</li></ol><Link to="/Auth" className="cm-text-link">לכניסה למערכת <ArrowLeft size={18} /></Link></article></div></section>
        ...
        <section className="cm-section cm-specialties"><div className="cm-wrap"><span className="cm-kicker">לכל כלי יש דרך עבודה משלו</span><h2>ניהול אופנועים, משאיות וכלי צמ״ה{' '}<br />לא רק רכב פרטי.</h2><div className="cm-specialties-grid">{categoryCards.map(card => <Link key={card.slug} to={card.href}><div className="cm-category-art"><img src={card.image} alt="" loading="lazy" width="1672" height="941" /></div><h3>{card.title}</h3><p>{card.text}</p><span>לפרטים <ArrowLeft size={17} /></span></Link>)}</div></div></section>
        {business}
