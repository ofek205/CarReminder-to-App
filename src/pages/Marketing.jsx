...
import { ArrowLeft, Loader2, Pencil, BellRing, BriefcaseBusiness, Car, Database, FileText, LockKeyhole, Mail, Menu, Search, ShieldCheck, Ship, Smartphone, Sparkles, Trash2, Users, Wallet, X } from 'lucide-react';
...
export default function Marketing() {
  ...
  return <div className="cm-site" dir="rtl">
    ...
    <main id="cm-main">
      {home && <>
        <section className="cm-hero"><MarketingHeroBackground /><div className="cm-wrap cm-hero-grid"><div className="cm-hero-copy"><span className="cm-kicker">הרכב, האופנוע וכלי השיט שלכם</span><h1>הטסט, הביטוח והטיפול הבא.{' '}<br /><em>הכול במקום אחד.</em></h1><p className="cm-hero-lead">Car Reminder מרכזת מועדים, מסמכים והוצאות ומזכירה לכם לפני שמגיע הזמן.</p><div className="cm-actions"><a href="#download" className="cm-button cm-gold">מורידים את Car Reminder <ArrowLeft size={19} /></a><a href="#check" className="cm-text-link">בדיקת רכב בלי הרשמה <Search size={17} /></a></div><div className="cm-hero-notes" aria-label="פרטים חשובים"><span><Database size={16} /> מידע ממקורות ציבוריים</span><span><Smartphone size={16} /> זמינה ב־App Store וב־Google Play</span><span><Mail size={16} /> תמיכה ישירה בעברית</span></div></div><div className="cm-hero-visual"><div className="cm-orbit" /><MarketingPhone src="/marketing/dashboard.webp" alt="לוח הבקרה באפליקציה עם נתוני הדגמה" priority /><span className="cm-photo-note">מסך מהאפליקציה במסגרת להמחשה</span></div></div></section>
      ...
      {!checkPage && !childReminderPage && <section id="download" className="cm-download"><div className="cm-wrap cm-download-simple"><img src={logo} width="64" height="64" alt="" loading="lazy" /><span className="cm-kicker">הצעד הבא שלכם</span><h2>הרכב הראשון שלכם{' '}<br /><em>יכול להיות מסודר כבר היום.</em></h2><p className="cm-download-lead">מורידים את Car Reminder, מוסיפים כלי ומרכזים את המועד הבא במקום אחד.</p><StoreLinks /><Link className="cm-text-link" to="/Auth">מעדיפים דפדפן? לכניסה באתר <ArrowLeft size={17} /></Link></div></section>}
    </main>
