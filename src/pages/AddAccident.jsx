...
export default function AddAccident() {
  ...
  // Apply a single lookup result (fields bag) to the other-driver
  // form fields. Factored so the multi-match dialog can re-enter it
  // after the user picks one of the candidates.
  const applyLookupFields = (fields) => {
    const updates = {
  ...
  const chooseMultiMatch = (idx) => {
    ...
    if (chosen?.fields) {
      applyLookupFields(chosen.fields);
    } else {
