...
export default function MarketingProductPage({ product }) {
  ...
  return <article className="cm-section cm-wrap cm-product-page">
    <div><Link className="cm-text-link" to="/website">חזרה לאתר <ArrowLeft size={17} /></Link><h1>{product.title}</h1><p className="cm-article-lead">{product.text}</p>
    ...
    <aside className="cm-specialty-aside">
      ...
      {!!related.length && <nav className="cm-related" aria-label="עמודים קשורים"><h2>עוד על ניהול הכלים</h2>{related.map(page => <Link key={page.slug} to={`/website/${page.slug}`}>{page.title} <ArrowLeft size={16} /></Link>)}</nav>}
      {!!product.relatedGuides?.length && <nav className="cm-related" aria-label="מדריכים קשורים"><h2>מדריכים שימושיים</h2>{product.relatedGuides.map(slug => <Link key={slug} to={`/website/guides/${slug}`}>{guides.find(guide => guide.slug === slug)?.title} <ArrowLeft size={16} /></Link>)}</nav>}
    </aside>
  </article>;
}
