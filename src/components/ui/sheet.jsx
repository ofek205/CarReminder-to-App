...
const SheetOverlay = React.forwardRef(({ className, ...props }, ref) => (
  <SheetPrimitive.Overlay
    ...
    // bottom nav.
    className={cn(
      "fixed top-0 inset-x-0 bottom-[56px] lg:bottom-0 bg-black/80 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0",
      className
    )}
    style={{ zIndex: 10000 }}
    {...props}
  ...
  "fixed gap-4 bg-background p-6 shadow-lg transition-transform ease-in-out data-[state=closed]:duration-300 data-[state=open]:duration-500",
  {
    variants: {
      side: {
        ...
        left: "top-0 bottom-[calc(56px+env(safe-area-inset-bottom,0px))] lg:bottom-0 left-0 w-3/4 border-r data-[state=closed]:-translate-x-full data-[state=open]:translate-x-0 sm:max-w-sm",
        right:
          "top-0 bottom-[calc(56px+env(safe-area-inset-bottom,0px))] lg:bottom-0 right-0 w-3/4 border-l data-[state=closed]:translate-x-full data-[state=open]:translate-x-0 sm:max-w-sm",
      },
    },
    defaultVariants: {
      side: "right",
