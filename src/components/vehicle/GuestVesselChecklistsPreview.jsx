    ...
    <div className="rounded-2xl overflow-hidden" dir="rtl"
      ...
      <div className="px-4 py-3 flex items-center gap-2"
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-bold" style={{ color: C.text }}>צ׳ק ליסטים לים</p>
          ...
          return (
            <div key={phase.key}>
              <button
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-bold" style={{ color: C.text }}>{phase.label}</p>
                  <p className="text-[11px]" style={{ color: C.gray400 }}>{phase.items.length} פריטים</p>
                </div>
                {open
                  ...
                  {phase.items.map((item, i) => (
                    <div key={i}
                      ...
                      <div className="w-5 h-5 rounded-md border-2 shrink-0 flex items-center justify-center"
                        style={{ borderColor: C.gray300, background: C.gray50 }}>
                        <Check className="w-3 h-3" style={{ color: C.gray300 }} />
                      </div>
                      <span className="text-xs flex-1" style={{ color: C.gray700 }}>{item}</span>
                    </div>
                  ))}
                </div>
              )}
      ...
      <Link to={createPageUrl('Auth')}
        ...
        style={{ background: C.yellowSoft, color: C.warnDark, borderTop: `1px solid ${C.warnBorder}` }}>
