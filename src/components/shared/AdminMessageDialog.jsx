      ...
      <div className="fixed inset-0 z-[61] flex items-center justify-center p-4" onClick={onClose}>
        ...
        >
          <div className="flex items-center gap-2 px-4 py-3" style={{ background: '#DBEAFE', borderBottom: '1px solid #BFDBFE' }}>
            ...
            <button onClick={onClose} className="w-7 h-7 rounded-lg flex items-center justify-center hover:bg-blue-200/50 transition">
              <X className="w-4 h-4" style={{ color: '#1D4ED8' }} />
            </button>
          </div>
          <div className="px-4 py-3">
            <p className="text-[14px] font-semibold mb-1" style={{ color: '#1C2E20' }}>{title}</p>
            {timestamp && formatTime && (() => {
              const formatted = formatTime(timestamp);
              return formatted ? <p className="text-[11px] mb-3" style={{ color: '#8B9C8E' }}>{formatted}</p> : null;
            })()}
            <p className="text-[13px] leading-relaxed whitespace-pre-wrap" style={{ color: '#374151' }}>{body}</p>
          </div>
