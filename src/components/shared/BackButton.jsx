/**
 * BackButton — small inline "→ חזור" affordance for page headers.
 *
 ...
 * back gesture on iOS/Android in RTL apps.
 */
...
export default function BackButton({ to = null, label = 'חזור', className = '' }) {
  ...
  return (
    <button
      type="button"
      onClick={handleClick}
      className={`inline-flex items-center gap-1 text-xs font-bold text-gray-500 hover:text-gray-900 active:scale-95 transition-all py-1.5 px-2 -mr-2 rounded-lg ${className}`}
      aria-label={label}
    >
