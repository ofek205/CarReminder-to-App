...
הדרך הנכונה קדימה: `pg_dump --schema-only` מפרודקשן כ-baseline, ארכוב 199 הקבצים כתיעוד פורנזי, ופנקס בתוך ה-DB שמתעד `filename`, `sha256` של התוכן בזמן ההחלה, `applied_at`, ו-`target_database`. ה-hash הוא השדה הנושא — הוא מה שתופס „הקובץ השתנה אחרי שהחלתי אותו”.
