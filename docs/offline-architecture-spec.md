...
- [~] **ONLINE-REQUIRED** (`offlineCapable:false`): ✅ sharing (5) + members/team/business (7) via `access.js` (`54c4457`); ✅ drivers + assignments (6) via `drivers.js` (`4f1aced`); ✅ route.createWithStops. TODO: community (11), admin/view-as (~20), `vehicle.deleteWithShareChoice`, `notify_*` side-effects, `appNotification` bulk mark-read, `deviceToken.register`, `account.{ensure,claimMigrated,deleteMine}`, `vehicle.bulkAdd`, driver mileage/event RPCs
