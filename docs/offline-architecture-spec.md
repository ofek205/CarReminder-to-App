...
- [ ] Known gap (cosmetic, deferred): envelope call sites render *their own* generic failure copy (e.g. "שמירה נכשלה") rather than the `OfflineError` message, so offline there reads as a generic error. Fixing it means touching ~30 call sites to check `isOfflineError(error)`; better done alongside Phase 6's pending-sync UI than as a standalone sweep.
