...
### Phase 3 — outbox core (first real offline writes) — ENGINE COMPLETE, coverage deliberately narrow
- [x] **`src/lib/dal/outbox.js`** — durable queue in its OWN IndexedDB database (`cr-outbox`), not a second key in the disposable RQ cache store: `buster` wipes that on every release and `clearPersistedCache` deletes it, and unsynced writes are not disposable. Uses idb-keyval's `update()` so every mutation is a **single read-modify-write transaction** — a plain get-then-set would let two tabs drop one another's writes, the same hazard D-3 documents for the cache, which is not acceptable for a user's unsynced data. Items carry `{opId, command, payload, userId, localId, createdAt, attempts, status, lastError}`, FIFO. **Every item stamps its OWNER's user id** rather than scoping the store by it, because a store key is fixed at module load before auth resolves — the exact trap that made the persister's first identity fix wrong. Cap of 200 evicts the oldest PENDING item and **never a FAILED one**; a queue that is all failures refuses the new write instead, so the caller falls back to an honest refusal rather than discarding someone's unresolved one.
- [x] **`src/lib/dal/sync.js`** — drain on reconnect, strictly serial and ordered (a create then an update to the same row must arrive in that order). The real content is `classify()`, which sorts every outcome into SUCCESS / RETRY / TERMINAL / PAUSE with the reasoning next to each code: `42501` RLS and `23503`/`PGRST116` missing-row are TERMINAL; **`23505` unique-violation is SUCCESS**, because on a replay it almost always means the write landed just as the connection dropped and we are seeing our own row (without that rule a successful save is retried forever and then reported lost); auth trouble PAUSES and never marks anything failed; anything unrecognised RETRIES, since an extra request is cheaper than wrongly parking a write. Retries stop at `MAX_ATTEMPTS` 5 and become review items, because retrying silently for days is its own way of losing something.
- [x] **A queued write leaves the engine only two ways:** the server confirmed it, or it is FAILED and kept with its error for review (§5.4). There is no path that drops one quietly.
- [x] **`src/lib/dal/queueWrite.js` + the `runCommand` hook** — offline, a queueable write is enqueued and RESOLVES in the shape the call site already handles, so a queued create looks like the success it is. Optimistic row carries `_pendingSync` and a `local_` id.
- [x] **Outbox cleared at all 5 identity boundaries** by folding `clearOutbox()` into `clearPersistedCache()` — same reasoning as its memory-then-disk order: a future call site cannot forget half of it. It correctly inherits the `event !== 'INITIAL_SESSION'` guard, so an offline token expiry does **not** wipe queued work.
- [x] **28 new tests**, all three suites mutation-tested. The outbox eviction test **found a real bug in my own implementation on first run**: `slice(1)` dropped the first item regardless of status, so it evicted a FAILED item — exactly the data a human still needed. Also caught: ignoring the envelope contract in the drain, and removing the stop-on-failure `break`.
- [ ] 🛑 **COVERAGE IS 3 COMMANDS ON PURPOSE, not timidity.** Only `corkNote.create` / `task.create` (plus purely-local edits of a row they are still holding) are queued, out of 52 `offlineCapable` commands. The two DB prerequisites decide this: **conflict detection needs `updated_at`** and `vehicles` has none, so queueing an UPDATE to a server row means a write queued hours ago silently overwriting newer data — the exact "silently corrupt data" class this whole refactor closed. And **client-supplied ids on insert** are unverified against each table's RLS `WITH CHECK`, so none is sent: the server assigns the id and `invalidates` refetches to replace the optimistic row. A pure insert of an owner-scoped row needs neither prerequisite, which is why it is the unblocked set.
- [ ] 🛑 **Ofek, to widen it:** (1) `ALTER TABLE ... ADD COLUMN updated_at timestamptz DEFAULT now()` + a `BEFORE UPDATE` trigger on every offline-write table, then LWW detection can compare it; (2) confirm a client-supplied `id` passes RLS on those tables if optimistic creates should keep a stable id across the flush.
- [ ] 🛑 **The §10.1 warning is a prerequisite before widening.** `clearOutbox()` on logout discards unsynced writes, which is the decided policy — but the warning that makes it a *choice* is UI and belongs to Phase 6. Today two things keep it from being silent data loss: the queue drains the moment connectivity returns so it is normally empty, and the INITIAL_SESSION guard means a token expiry does not wipe it. With a wider command set that stops being good enough.
- [ ] Not live-tested end to end: `queueWrite` needs a real session, so the signed-in offline-create-then-reconnect flow is Ofek's to exercise. Every layer beneath it is unit-tested.

### Phase 4 — declaration-driven coverage (2026-09-08)
- [x] **Offline behaviour now lives on the COMMAND, not in the outbox.** Each command declares `outboxOp: 'insert' | 'update' | 'delete'`, and `queueWrite.js` reads it. Enabling a command is a one-line addition to `OUTBOX_ENABLED` instead of a new special case, and the hardcoded per-command map (with its `creates: [...]` links) is gone.
- [x] **A pending create is found by `table`, not by command name.** `corkNote.create` and `task.create` both write `cork_notes` and nothing in their names says so, so a task edited offline has to be able to find whichever create is holding its row. The queue item records its `table` for this.
- [x] 🛑 **Fixed a real cross-vehicle leak found while verifying the actual query keys.** The keys are parameterised — `['cork-notes', vehicleId]`, `['tasks-v2', vehicleId]` — and `setQueriesData` matches by PREFIX, so patching `['cork-notes']` prepended a new note to **every vehicle's cork board**. Three defences now, because relying on every future declaration being careful is not one: the key is resolved against the ROW (`invalidates(row)`), a key containing an `undefined` part is refused outright, and the patches use `exact: true` — so a command that declares only a bare prefix patches NOTHING instead of everything. The drain's `invalidateQueries` keeps prefix matching, where hitting every related list is exactly what is wanted.
- [x] **Half-registration is now structurally impossible.** A test asserts every name in `OUTBOX_ENABLED` declares an `outboxOp`, and that every enabled INSERT declares `invalidates` (without it the optimistic row keeps its `local_` id forever, since nothing refetches to replace it). Mutation-verified: removing `outboxOp` from `task.create` fails the test and names it.
- [ ] **Finding that shapes the rest of Phase 4:** a derived optimistic key only works where the list key is reconstructible from the write payload. It is for cork notes and tasks (`[key, vehicle_id]`). It is NOT for `documents`, whose real key is `['documents', accountId, vehicleIdParam, restrictToDriverAssignments, driverAssignedVehicleIds]` — a payload cannot know `restrictToDriverAssignments`. So broad domains need an explicit per-command `optimistic(queryClient, row)` function, which is work that belongs WITH enabling each command rather than as a speculative sweep now. The registry has always declared `optimistic` separately from `invalidates` for exactly this reason; the derived-key path is only a convenience default for simple keys.
- [ ] 🛑 **The enabled set is unchanged, and still gated on three things**: `updated_at` + trigger (for updates to server rows), client-id RLS verification (only if optimistic creates should keep a stable id across the flush), and the §10.1 logout warning (more enabled commands means more that `clearOutbox` can discard silently).

### Phase 5 — file-upload queue (2026-09-08) — MECHANISM COMPLETE, not yet wired to a screen
- [x] **The spec's "pending storage_path then patch the row" is not needed.** `buildStoragePath` generates the path client-side from a v4 uuid, so the FINAL path is knowable while offline: the row is created once carrying its real permanent path, and the file is later uploaded to exactly that path. No placeholder, no second write, and no window where the row holds a value meaning "not really". `uploadToBucket` now delegates to a new `uploadToPath(file, path, {sign})`, so existing callers are byte-for-byte unchanged.
- [x] 🛑 **`sign: false` on the queued upload is a correctness fix, not an optimisation.** `uploadToPath`'s signing-failure branch DELETES the uploaded object, on the sound reasoning that the caller never received the path so it would be an untrackable orphan. For a queued upload that reasoning inverts: the row already points at that exact path, so deleting would destroy a successful upload the database references. Queued uploads skip signing entirely; `useSignedUrl` / `refreshSignedUrl` re-sign on read anyway.
- [x] **`dal/blobStore.js`** — blobs in their OWN IndexedDB database, keyed by `storage_path` (already the stable id shared by row, queue item and blob, so nothing needs a second key). Stored as real Blobs via structured clone, so **no base64 anywhere**: `assertNotBase64` / `guardFileFields` keep working untouched and nothing large reaches Postgres. Separate from the outbox because an outbox item is re-serialised as JSON on every queue mutation, and a megabyte photo inside one would be rewritten repeatedly. Caps at 15 MB per file / 60 MB total, and **refuses rather than evicts** — sacrificing an earlier pending photo for a new one is the one thing this store must not do. IDB timeout raised to 5s (vs the cache's 2.5s) because blobs are large.
- [x] **`dal/commands/storage.js`** — uploads are now commands, closing the last server interaction that sat outside the seam. The blob is NOT in the payload; the item carries only the path. A missing blob (quota eviction, cleared store) throws a code the drain classifies as TERMINAL, so it goes to review instead of retrying against a file that no longer exists.
- [x] **The local copy is dropped only after the upload confirms.** Mutation-verified: reversing that order fails the "keeps the blob when the upload fails" test. A failed attempt must never consume the only copy of the user's photo.
- [x] **`dal/uploadQueue.js` — `attachFile({file, pathPrefix})`** returns the same `{file_url, storage_path}` shape online or offline, so a screen that knows how to put a path on a row needs no offline branch. Offline `file_url` is a local object URL, which is how §5.6's "render from the local blob until upload completes" is satisfied — by the return value, not by per-component special cases. Every failure path THROWS rather than returning a path, because a row pointing at a file that exists nowhere is permanently broken while a refusal is retryable. **Callers must revoke the object URL on unmount.**
- [x] Blobs cleared at all 5 identity boundaries alongside the outbox: a photo of someone's vehicle documents is customer data at rest, and larger and more sensitive than the queue record referencing it.
- [x] 9 tests, mutation-verified.
- [ ] 🛑 **Deliberately NOT wired into any call site yet, and wiring it early would make things worse.** The existing upload sites still call `uploadToBucket`. If `attachFile` were substituted now, an offline attach would store the blob and queue the upload while the accompanying row create is still refused (documents/photos are not in `OUTBOX_ENABLED`) — leaving an orphan blob and a queued upload with no row pointing at it. That is strictly worse than refusing both. Wiring belongs with enabling each command's create, i.e. after the same three gates.

### Phase 6
### Phase 6 — surfacing unsynced work (2026-09-08)
- [x] **🛑 THE §10.1 LOGOUT WARNING EXISTS, so the gate that blocked widening coverage is now CLEAR.** `hooks/useLogoutWithGuard.js` warns before discarding, and **both** `handleLogout` copies in Layout.jsx call it. That mattered more than the wording: a guard on one route and not the other is worse than none, because the unguarded route looks safe while destroying data. The duplicated localStorage cleanup collapsed into the same hook. It **fails toward warning** when the outbox cannot be read, with its own honest copy, because a false "nothing pending" is the failure mode that loses work. Confirm verb is `התנתק ומחק`, not `התנתק` — the familiar word alone would hide the deletion at the only moment the user can still choose otherwise. Reuses `ConfirmDeleteDialog`; no new component.
- [x] **The banner reports the outbox, not just connectivity**, because "is my data current" and "did my change save" are one question to a user. Extending the single strip rather than adding a second is deliberate. Five states, and the notable one is what is NOT shown: an online queue only speaks up once it has **lingered** (6s), since a normal drain finishes in well under a second and a strip flashing on every reconnect would shove the page down and back. Neutral grey for offline and delayed sync (states, not errors, and yellow/orange are taken by the staging and view-as banners); **danger only for a terminal failure**, the one state needing a decision.
- [x] **`bannerState` extracted as a pure selector, which is how all five states are genuinely covered** — the banner renders only on signed-in pages, so its states are unreachable in a preview without a real session. The table immediately earned it, catching `שינוי אחד לא נשמרו`: singular noun with a plural verb, because the count helper handled the noun while the verbs were hardcoded plural. Hebrew needs agreement on both, and that is exactly the class of error a visual pass ships.
- [x] `hooks/useOutboxStatus.js` — a 3s poll rather than a subscription, deliberately: the outbox is written from non-React code (run.js, sync.js) and **from other tabs**, so an in-tab store subscription would miss most of what it needs to reflect.
- [x] 13 tests. Preview verified: boots clean, banner correctly absent for an online user with an empty queue, no new console errors.
- [ ] **Retry/discard sheet NOT built, deliberately.** The banner reports the failure COUNT, which is what closes "never silently drop" — the user is told. The *actions* need a new surface that is empty until coverage widens, so `onShowFailed` is the wired seam and today nothing passes it, meaning the `הצג` affordance does not yet render. Build it together with the first widening.
- [ ] Conflict UI: not started, and correctly so — it has nothing to resolve until `updated_at` exists and updates to server rows can be queued at all.
- [ ] Note: §10.1's warn+discard is implemented as decided. Flush-then-logout, which the spec had originally recommended and which would lose less data, is a small change inside that one hook if Ofek revisits it.
