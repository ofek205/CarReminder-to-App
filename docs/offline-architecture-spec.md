...
- [~] **ONLINE-REQUIRED** (`offlineCapable:false`): ✅ sharing (5) + members/team/business (7) via `access.js` (`54c4457`); ✅ route.createWithStops. TODO: drivers (external-driver + assignment RPCs, 6), community (11), admin/view-as (~20), `vehicle.deleteWithShareChoice`, `notify_*` side-effects, `appNotification` bulk mark-read, `deviceToken.register`, `account.{ensure,claimMigrated,deleteMine}`
