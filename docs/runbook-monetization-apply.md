...
> ## ✅ ההחלה הושלמה 2026-09-10. מה שנשאר הוא הדלקת הדגלים
>
> **תשעת הקבצים הוחלו ונרשמו בפנקס.** `public.sql_ledger` מחזיק רשומה אחת לכל אחד, עם ה-sha256 של הבייטים שרצו בפועל ועם `p_notes` שאומר מה אומת ומה **לא**. אימות: `records = 1` לכל אחד מהשמונה, בלי חסרים ובלי כפילויות.
>
> **הגרנדפאדר בוצע:** 21 חשבונות הוקפאו על הכמות שהחזיקו, 0 אי-התאמות מול הספירה בפועל. המספרים זהים להתפלגות שנמדדה יום קודם, כלומר לא נוצר סחף בין ההחלטה לביצוע.
>
> **כל שישה הדגלים כבויים**, ולכן ההתנהגות למשתמשים לא השתנתה. סדר ההחלה שלמטה נשאר כתיעוד של מה שקרה, ובשביל סביבה עתידית.
>
> **⚠️ שלב 2ב הוחל בשני מעברים.** הראשון נכשל על `syntax error at or near "public"` בשורה 549, שורת `update` כפולה ב-`admin_extend_grace`, והשאיר החלה חלקית כי לקובץ אין `BEGIN`. תוקן ב-`348a661` והורץ שוב במלואו. השער שמונע חזרה של המחלקה הזו הוא `scripts/sql-duplicate-statements.cjs`, ורץ ב-pre-push וב-`npm test`.
>
> **מה שנשאר, ואינו SQL:** פריסת `ai-proxy` (בלעדיה מכסת ה-AI אינה בנתיב בכלל), הדלקת דגלים **אחד בכל פעם**, וההחלטה על ספק סליקה ישראלי שחוסמת את פאזות 6-7.
...
> ⚠️ **אלה ערכי ה-זרע, ולא הערכים החיים.** ב-2026-09-11 הועלו החינם ל-**10** ו-₪9 ל-**15** ב-`UPDATE`. הזרע נשאר 5/10/30/NULL כי הקובץ הוחל ונרשם בפנקס ואין לערוך אותו. **המקור החי הוא `plan_limits`.** בסביבה חדשה שמחילה מאפס, הערכים למעלה הם הנכונים.
...
> ⚠️ **הפסקה הבאה היא היסטוריה.** ההכרעה ב-2026-09-11 הייתה להשאיר את החינם על 10 ולא להדק ל-5, ולכן הדלי הזה כבר אינו קוהורט בסיכון. נשמר כתיעוד של השאלה ששאלנו.

**הדלי `6-10` היה הליבה:** אלה חשבונות שתקינים **היום** (התקרה האישית בפועל היא `app_config.personal_vehicle_cap` שברירת המחדל שלה **10**) ויהיו **מעל** תקרת חינם של 5. הצ'ארט ב-Admin Analytics לא עונה על זה כי `TAIL = 10` ב-`AdminAnalytics.jsx` מקבץ הכל מעל 10 לעמודה אחת.
