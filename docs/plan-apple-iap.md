...
4. 🟠 **Streamlined Purchasing is ON and cannot be turned off yet** (Ofek's App Store Connect, 2026-09-26: "cannot be turned off because your latest approved binary doesn't include the required StoreKit APIs"). Apple needs an approved binary that handles `PurchaseIntent`; `@capgo/native-purchases` 8.7.0 listens only to `Transaction.updates` (NativePurchasesPlugin.swift:46), no PurchaseIntent. While it is on, a purchase started on the App Store (promoted IAP, win-back offer, contingent pricing) completes with no `appAccountToken`, so `linkProblem()` returns `no_account_token` and the buyer pays for a plan never granted. Offer codes bypass the app even with it off. **Mitigation until fixed: promote nothing on the product page, create no win-back offers and no offer codes** (runbook stage B). Fix: handle `PurchaseIntent.intents` natively and purchase with the account token, ship it, then turn it off; or link such purchases server-side with Apple's Set App Account Token endpoint.
5. 🟠 **Multiseat purchases are ON by default per subscription** (Apple, WWDC26; Volume Purchasing via Apple Business / School Manager launches 2026-10-22, Group Purchases this winter). A seat bought by an organization carries no token for any of our accounts and arrives with ownership `.assigned`, so it is never granted. Runbook stage B: each subscription's Purchase Options → "No, don't allow multiseat purchases" + "The App Store". Supporting it later means a seat-to-account link, which we do not have.
