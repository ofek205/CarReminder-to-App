-- ==========================================================================
-- redeem_invite_token: allow 'driver' — 2026-06-27
--
-- T2.8 + the account-request invitees let managers invite a DRIVER. For a
-- REGISTERED driver this works (PATH A: pending row + accept). For an
-- UNREGISTERED driver, PATH B creates an `invites` token with
-- role_to_assign='driver', but redeem_invite_token's strict whitelist was
-- ('מנהל','שותף') only — so the driver hit `invalid_invite_role` and could
-- NOT join. This adds 'driver' to the whitelist.
--
-- (Still rejects 'בעלים' and garbage — ownership is never handed out via a
-- token, per audit #18.)
--
...
begin
  ...
  -- Reject invalid roles instead of silently coercing. 'driver' is now a
  -- valid invite role (fleet drivers); 'בעלים'/garbage are still rejected —
  -- ownership is never handed out via a token.
  if inv.role_to_assign not in ('מנהל', 'שותף', 'driver') then
